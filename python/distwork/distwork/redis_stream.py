...
class RedisStream:
  def __init__(self, redis_url='redis://127.0.0.1:6379', task_stream='tasks',
               task_group='taskgroup', maxlen = 1000000):
  ...
  def enqueue_work(self, work: dict) -> bool:
    try:
      self._redis.xadd(self._task_stream, work)
      return True
    except Exception as e:
      print(e)
      return False
  ...
  def dequeue_work(self, worker_func: Callable, consumer_id :str, max_work:int =5):
    works = self._redis.xreadgroup(self._task_group, consumer_id, {self._task_stream : '>'},
                                   max_work, 10000)
    if not works:
      return True
